    ...
    impl RWAFactoryImpl of IRWAFactory<ContractState> {
        ...
        fn create_rwa(
            ...
            if fee > 0_u256 {
                ...
                let success = fee_token.transfer_from(
                    caller,
                    starknet::get_contract_address(),
                    fee
                );
                assert!(success, "Factory: fee payment failed");
            ...
            // Link token to vault (token needs vault address for mint/burn authorization)
            let token = IRWATokenDispatcher { contract_address: token_address };
        ...
        fn withdraw_fees(ref self: ContractState, recipient: ContractAddress) {
            ...
            let success = fee_token.transfer(recipient, fees);
            assert!(success, "Factory: fee transfer failed");
