// src/server.ts — RWA Oracle Admin API + Shielded Vault Privacy Layer (PRODUCTION)
...
import { PrivacyEventListener } from "./privacy/eventListener";

// ── Privacy event listener singleton ─────────────────────────
// Started once and shared across requests via closure.
let privacyListener: PrivacyEventListener | null = null;

export async function startPrivacyListener(): Promise<PrivacyEventListener> {
  const rpcUrl       = process.env.STARKNET_RPC_URL!;
  const vaultAddress = process.env.VAULT_CONTRACT_ADDRESS ?? "";
  const wsPort       = parseInt(process.env.PRIVACY_WS_PORT ?? "8080", 10);

  const listener = new PrivacyEventListener({
    rpcUrl,
    vaultAddress,
    wsPort,
  ...
  if (vaultAddress) {
    ...
    logger.info({ vaultAddress, wsPort }, "Privacy event listener started");
  } else {
    logger.warn("VAULT_CONTRACT_ADDRESS not set — privacy listener in stub mode");
  }
...
export function createAdminServer(listener?: PrivacyEventListener) {
  const app = express();
  ...
  // ── Privacy layer — no auth required (public Merkle data) ────
  if (listener) {
    app.use("/api/privacy", createPrivacyRouter(listener));
    logger.info("Privacy API routes mounted at /api/privacy");
  }
...
export async function startAdminServer(): Promise<void> {
  const listener = await startPrivacyListener();
  const app  = createAdminServer(listener);
